...
function buildAnalysisPrompt(d) {
  ...
  L.push('- 示例："INT和对局相关点位，以及活动点位频次有所增长，带动大盘"');
  L.push('');
  L.push('**其他格式规则：**');
  ...
  L.push('```');
  L.push('UNO');
  L.push('整体：收入 -1.90%，DAU 基本稳定，频次小幅提升；但 eCPM(log) 与 AdARPU 下滑');
  L.push('平台：双端 ARPU 均有下滑，主要受eCPM下降影响，DAU频次小幅增长');
  L.push('渠道：双端eCPM均下滑，且主要受AL渠道影响');
  L.push('点位：INT和对局相关点位，以及活动点位频次有所增长，带动大盘');
  L.push('');
  L.push('UNO2');
  L.push('整体：收入下滑 -5.54%，DAU 继续缩量叠加 eCPM(log) 大幅回落是双重拖累；频次明显提升（主要来自 Banner 广告结构变化）部分对冲，但未能扭转收入下行。');
  L.push('平台：双端 ARPU 小幅下滑，主要受到eCPM下降影响，DAU频次有所提升');
  L.push('渠道：双端eCPM均下滑，AL权重影响最大，影响大盘');
  L.push('点位：在线时长小幅 +2.37%；受 DAU 下滑影响，gamefinish 频次有所下降，但 Banner 广告频次增长带动整体频次提升。');
  L.push('');
  L.push('P10');
  L.push('整体：收入 -4.01%，频次与 eCPM(log) 同步走弱，AdARPU 明显下行；DAU 基本持平。');
  L.push('平台：双端 ARPU 均有下滑，AND 端降幅更大，IOS 端 ARPU 也有所回落，主要受 eCPM 下行影响，其次DAU频次也有小幅下滑。');
  L.push('渠道：双端 applovin 下滑导致大盘下滑。');
  L.push('点位：在线时长小幅回落 -1.87%；Gameplay 插页和 Drawcard 活动频次均有下降');
  L.push('');
  L.push('SKB');
  L.push('整体：收入-4.31%，频次明显下滑是收入下降核心，eCPM(log) 基本持平，DAU 稳定。');
  L.push('平台：双端 ARPU 均有下滑，主要是DAU频次下滑导致，集中在插屏广告。');
  L.push('渠道：iOS主要是AL增长拉高大盘，And虽然AL也有增长，但Unity和Moloco下滑起到负向对冲');
  L.push('点位：在线时长小幅回落 -2.24%；基本全点位有下滑趋势，插屏降幅较为明显');
  L.push('```');
  L.push('');
  L.push('### 分析规则（SKILL）');
