...
function buildWeeklyFullPrompt(d) {
  ...
  L.push('');
  L.push('**2.5 双聚合数据**');
  L.push('- MAX vs Admob 的 ARPU gap 变化');
...
function buildWeeklyDataPrompt(d) {
  ...
  L.push('- **链接**：https://omnieye.mattel163.com/index.html#/v3/adverAna/keytrend');
  L.push(`- **数据类型**：${d.data_type === 'log' ? 'Log（客户端打点）' : 'API（渠道回传）'}`);
  L.push(`- **日期范围**：本周一 ~ ${d.report_date}（周日），以及上周一 ~ 上周日（对比周）`);
  L.push(`- **产品**：${d.products.map(p => `${p}(${productMap[p]})`).join('、')}`);
  L.push('');
  L.push('### 需要调用的接口');
  L.push('');
  L.push('按产品分别调用以下接口（每产品各调一次）：');
  L.push('');
  L.push('- `adverAnakeytrendTable` — 汇总报表（IAA 日均收入、DAU、ARPU、eCPM、频次）');
  L.push('- `adverAnakeytrendTable` — 数据概况（含 RV/INT/Banner 子项）');
  L.push('- `adverAnakeytrendRevenue` — 整体数据（分平台收入/ARPU/在线时长）');
  L.push('- `adverAnakeytrendeCPM` — 平台数据（分 RV/INT Freq/DAU、eCPM）');
  L.push('- `adverAnakeytrendTableAdchannel` — 渠道数据（需分 AND/IOS 各调一次）');
  L.push('- `adverAnakeytrendTablePointLog` — 点位数据（需分 AND/IOS 各调一次）');
  L.push('');
  L.push('### 公共参数模板');
  L.push('```json');
  L.push(`{"adchannel": ["all"], "adtype": ["all"], "country": ["All"], "dataType": "${d.data_type}", "dateEnd": "${d.report_date}", "dateStart": "本周一日期", "dateType": "date", "platform": ["All"]}`);
  L.push('```');
  L.push('');
  L.push('**注意事项**：');
  L.push('- 渠道数据和点位数据需额外用 `platform: ["ANDROID"]` 和 `platform: ["IOS"]` 各调一次');
  L.push('- 需同时拉取上周同期数据用于计算周环比');
  L.push('- 日期范围建议覆盖近 3 周，便于看趋势');
  return L.join('\n');
}
