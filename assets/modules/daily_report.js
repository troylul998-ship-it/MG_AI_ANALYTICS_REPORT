...
function buildAdvancedFullPrompt(d) {
  ...
  L.push('');
  L.push('## ⚠️ 执行前必须先向用户确认的 3 件事（换机器/换团队必变，禁止照抄示例值）');
  L.push('本 Prompt 可能在任意主机、任意用户账号下执行，以下信息**因环境而异**，动手前必须先反问用户，不得凭历史记忆或示例值臆造：');
  L.push('1. **测试群 chat_id**（先推这里验证效果）——问用户："请提供用于测试推送的飞书群 chat_id"');
  L.push('2. **正式群 chat_id**（确认无误后才推这里）——问用户："请提供正式推送的飞书群 chat_id，以及是否需要 @所有人 或 @指定成员（需成员 open_id）"');
  L.push('3. **本机 npm 全局包路径**——不要假设任何固定路径，执行时动态探测（见 Step 2 环境探测），若探测失败再问用户');
  L.push('> 下文出现的 chat_id / 人名 / 路径均为**占位示例**，仅用于说明格式，实际执行前必须替换为用户本次提供的真实值。');
  L.push('');
  L.push('## 📅 日期口径（写死）');
  L.push('```');
  ...
  L.push('**⚠️ 环境探测（每台机器用户名不同，禁止硬编码 C:\\Users\\<固定用户名>\\...）**：');
  L.push('```powershell');
  L.push('# 用 npm root -g 动态拿到全局包目录，与当前登录用户名无关，任何机器都适用');
  L.push('npm root -g');
  L.push('# 预期形如 C:\\Users\\<当前用户>\\AppData\\Roaming\\npm\\node_modules（<当前用户>因机器而异，不要猜测/硬编码）');
  L.push('# 验证依赖包存在：');
  L.push('Test-Path (Join-Path (npm root -g) "@aidea\\bi-analyse-client\\dist\\platformClient.js")');
  L.push('Test-Path (Join-Path (npm root -g) "@larksuite\\cli\\scripts\\run.js")');
  L.push('# 若任一为 False → 报警停止，向用户确认该机器是否已安装 @aidea/bi-analyse-client 和 @larksuite/cli，不得假设路径继续往下走');
  L.push('```');
  L.push('运行环境（路径变量由上面探测得到，不要写死用户名）：');
  L.push('```javascript');
  L.push("// NPM_ROOT = 上面 `npm root -g` 的输出（如 C:/Users/<当前用户>/AppData/Roaming/npm/node_modules）");
  L.push("import { PlatformClient } from 'file:///' + NPM_ROOT + '/@aidea/bi-analyse-client/dist/platformClient.js';");
  ...
  L.push('⚠️ **CHAT_ID 不得使用任何示例/历史群号**——本节的 chat_id、@成员 open_id 必须使用「执行前必须先向用户确认的3件事」里用户本次提供的真实值。若用户尚未提供，先停下来问，不要用记忆中的群号代替。');
  L.push('```javascript');
  L.push('// runJs 路径 = Step 2 环境探测得到的 NPM_ROOT + "/@larksuite/cli/scripts/run.js"');
  L.push("const runJs = NPM_ROOT + '/@larksuite/cli/scripts/run.js';");
  L.push("const TEST_CHAT_ID = '{{用户提供的测试群chat_id}}';   // 向用户确认，不要用旧示例");
  L.push("const PROD_CHAT_ID = '{{用户提供的正式群chat_id}}';   // 向用户确认，不要用旧示例");
  L.push("execFileSync(process.execPath,[runJs,'im','+messages-send','--as','bot','--chat-id',TEST_CHAT_ID,'--msg-type','interactive','--content',JSON.stringify(card)]);");
  L.push("// 确认无误后才推正式群；@提及仅在用户明确要求且提供了成员 open_id 时才发：");
  L.push('// atMsg = { text: \'<at user_id="{{用户提供的open_id}}">{{姓名}}</at> 日报已更新 ✅\' } 或 <at user_id="all">所有人</at>（仅当用户明确要求@所有人）');
  L.push('```');
  L.push('**顺序铁律**：先推测试群 → 用户确认无误 → 才推正式群（+用户指定的@对象，若有）。严禁跳过确认直推正式群，严禁使用未经用户确认的 chat_id 或 open_id。本流程不创建飞书文档。');
  L.push('');
  L.push('---');
  L.push('');
  L.push('## ✅ 推送前自检 & 收尾对抗审查');
